...
'altered the weights for the second sim on OC policy
  %u_weight = "1.0"
  %p_weight = "1.0"
  %r_weight = "10.0"
...
'append the data on epop and rgdpch
...
'*************************************************************
' Sim 1: optimal policy with pictrsh
'*************************************************************
...
'%algstr = contains information about the MC solution algorithm. The example selects the quasi-Newton algorithm, which is usually the fastest of the available algorithms when running a single FRB/US simulation. 
...
'***********************************************************
  ...
  call dateshift(%simstart,%graphend,25)

  smpl %graphstart %graphend
  graph fig1a.line rff rff{%sufcontrol} 
  ...
  fig1a.setelem(1) lcolor(red)  legend("Consensus Baseline") lwidth(1)
  fig1a.setelem(2) lcolor(black) legend("Optimal Control") lwidth(1)
  fig1a.addtext(t,just(c),font("arial",18)) Federal Funds Rate


  smpl %graphstart %graphend
  graph fig1b.line rgdpch rgdpch{%sufcontrol} 
  ...
  fig1b.setelem(1) lcolor(red)  legend("Consensus Baseline") lwidth(1)
  fig1b.setelem(2) lcolor(black) legend("Optimal Control") lwidth(1)
  fig1b.addtext(t,just(c),font("arial",18)) Year to Year % Change in Real GDP


  smpl %graphstart %graphend
  graph fig1c.line lur lur{%sufcontrol} 
  ...
  fig1c.setelem(1) lcolor(red)  legend("Consensus Baseline") lwidth(1)
  fig1c.setelem(2) lcolor(black) legend("Optimal Control") lwidth(1)
  fig1c.addtext(t,just(c),font("arial",18)) Unemployment Rate


  smpl %graphstart %graphend
  graph fig1d.line pic4 pic4{%sufcontrol} 
  ...
  fig1d.setelem(1) lcolor(red)  legend("Consensus Baseline") lwidth(1)
  fig1d.setelem(2) lcolor(black) legend("Optimal Control") lwidth(1)
  fig1d.addtext(t,just(c),font("arial",18)) PCE Inflation Rate (4-Quarter)

smpl %graphstart %graphend
  graph fig1e.line epop epop{%sufcontrol} 
  ...
  fig1e.setelem(1) lcolor(red)  legend("Consensus Baseline") lwidth(1)
  fig1e.setelem(2) lcolor(black) legend("Optimal Control") lwidth(1)
  fig1e.addtext(t,just(c),font("arial",18)) Employment to Population Ratio

smpl %graphstart %graphend
  graph fig1f.line pieci pieci{%sufcontrol} 
  fig1f.options size(7,4.2)
  fig1f.legend display -inbox position(3.9,0.3) font("arial",15)
  ...
  fig1f.setelem(1) lcolor(red)  legend("Consensus Baseline") lwidth(1)
  fig1f.setelem(2) lcolor(black) legend("Optimal Control") lwidth(1)
  fig1f.addtext(t,just(c),font("arial",18)) Annual Growth Rate of EI Hourly Compensation

smpl %graphstart %graphend
  graph fig1g.line rg10 rg10{%sufcontrol} 
  ...
  fig1g.setelem(1) lcolor(red)  legend("Consensus Baseline") lwidth(1)
  fig1g.setelem(2) lcolor(black) legend("Optimal Control") lwidth(1)
  fig1g.addtext(t,just(c),font("arial",18)) 10-Year Treasury Rate

smpl %graphstart %graphend
  graph fig1h.line picxfe picxfe{%sufcontrol} 
  fig1h.options size(7,4.2)
  fig1h.legend display -inbox position(3.9,0.3) font("arial",15)
  ...
  fig1h.setelem(1) lcolor(red)  legend("Consensus Baseline") lwidth(1)
  fig1h.setelem(2) lcolor(black) legend("Optimal Control") lwidth(1)
